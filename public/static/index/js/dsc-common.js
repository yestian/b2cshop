...
$(function(){
	...
	$("*[ectype='cateItem']").on('mouseenter',function(){
		var T = $(this),
			cat_id = T.data('id'),
			eveval = T.data('eveval'),//临时变量，让内容只加载一次
			layer = T.find("*[ectype='cateLayer']");

		if(eveval != 1){
			T.data('eveval', '1');
			/*加载中 loading*/
			layer.find("*[ectype='subitems_" + cat_id + "']").html('<img src="'+js_img_path+'/img/loadGoods.gif" width="200" height="200" class="lazy">');
			...
			$.ajax({
			   ...
			   success: function(data){
					var channels = $("*[ectype='channels_" + data.cat_id + "']"),
	...
	$("*[ectype='items'] *[ectype='item']").on('mouseenter',function(){
		...
		if(eveval != 1){
			T.data('eveval', '1');
			/*加载中by wu*/
			layer.find("*[ectype='subitems_" + cat_id + "']").html('<img src="/static/index/img/loadGoods.gif" width="200" height="200" class="lazy">');
			$.ajax({
			   type: "GET",
