...
class Index extends Base
{
    public function index(){
        ...
        if(cache('slider')){
            ...
            $slider=db('slider')->where('status',1)->select();
            if($this->config['cache']=='是'){
                cache('slider',$slider,$this->config['cachetime']);
            }
            
        }
        
        //调用公告及促销文章，缓存数据，清空缓存 cache(null);
        if(cache('ans')){
            $ans=cache('ans');
        }else{
            $ans=model('article')->getArts(33,3);
            if($this->config['cache']=='是'){
            cache('ans',$ans,$this->config['cachetime']);
            }
        }
        ...
        if(cache('cateress')){
            ...
            if($this->config['cache']=='是'){
            cache('cateress',$caterec,$this->config['cachetime']);//缓存一小时
            }
        }
